interface IProps {
  ...
  const Paginator = ({ page, pageCount, total, isLoading, onClickPrev, onClickNext }: IProps) => {
	return (
	  <div className="flex flex-col items-center space-y-5 mx-auto mt-10">
		<p className="text-sm text-gray-500 mx-3">
		  Page <span className="mx-1 font-semibold text-white text-md">{page}</span> to
		  <span className="mx-1 font-semibold text-white">{pageCount}</span> of
		  <span className="mx-1 font-semibold text-white">{total}</span> Records
		</p>
